...
- **跨笔记导航历史**：笔记间跳转后可通过状态栏 ← → 按钮或 `Alt+←` `Alt+→` 快捷回溯。

### 知识发现

- **目录大纲面板**：工具栏一键切换，自动提取标题并在侧栏展示可点击跳转的层级大纲。
- **知识图谱**：以力导向图可视化笔记间的引用关系网，按分类着色，点击节点直达笔记。

### 性能与基础设施

- **FTS5 全文搜索**：基于 SQLite FTS5 的增量索引，trigram tokenizer 支持中英文混合搜索，速度远快于旧版全量遍历。
- **版本去重存储**：blake3 哈希去重，内容未变时不重复存储版本，节省 80%+ 磁盘空间。
- **CLI 命令行接口**：支持 `--cli list|get|search|daily|create|export`，方便脚本批量操作和管道集成。
