...
- **跨筆記導航歷史**：筆記間跳轉後可透過狀態欄 ← → 按鈕或 `Alt+←` `Alt+→` 快捷回溯。

### 知識發現

- **目錄大綱面板**：工具列一鍵切換，自動提取標題並在側欄展示可點擊跳轉的層級大綱。
- **知識圖譜**：以力導向圖視覺化筆記間的引用關係網，按分類著色，點擊節點直達筆記。

### 效能與基礎設施

- **FTS5 全文搜尋**：基於 SQLite FTS5 的增量索引，trigram tokenizer 支援中英文混合搜尋，速度遠快於舊版全量遍歷。
- **版本去重儲存**：blake3 雜湊去重，內容未變時不重複儲存版本，節省 80%+ 磁碟空間。
- **CLI 命令行介面**：支援 `--cli list|get|search|daily|create|export`，方便腳本批次操作和管道集成。
